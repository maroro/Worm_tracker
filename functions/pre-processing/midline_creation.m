function midline_data = midline_creation(data)
...
%% use outline paths to create midline divided in X number of midpoints%%%%
    ...
    for p=1:2*round(data.SplitChannels.Body_Coords.worm_diam)
        ...
        if floor(Xplus(2))>1 && floor(Yplus(2))>1 && ...
          ceil(Xplus(2))<size(data.WorkingFrame.frame_mask_data.BW_Mask_Frame,2) && ceil(Yplus(2))<size(data.WorkingFrame.frame_mask_data.BW_Mask_Frame,1)...
          ...
          && data.WorkingFrame.frame_mask_data.BW_Mask_Frame(floor(Yplus(2)),floor(Xplus(2)))==1 ...
        ...
        if floor(Xminus(2))>1 && floor(Yminus(2))>1 && ...
          ceil(Xminus(2))<size(data.WorkingFrame.frame_mask_data.BW_Mask_Frame,2) && ceil(Yminus(2))<size(data.WorkingFrame.frame_mask_data.BW_Mask_Frame,1)...
          && norm([Xminus(2)-midline_data.midpoints(2,1) Yminus(2)-midline_data.midpoints(2,2)])>=...
              norm([X1(2)-Xminus(2) Y1(2)-Yminus(2)])...
          && data.WorkingFrame.frame_mask_data.BW_Mask_Frame(floor(Yminus(2)),floor(Xminus(2)))==1 ...
...
% organism's shape, posture, and movements over time.
    ...
    Xminus=Xminus+0.5*(c./sqrt(1+c.^2));
        ...
        for point=3:data.ROI_Input.Num_midpoints
            ...
            if floor(Xplus(point))>1 && floor(Yplus(point))>1 && ceil(Xplus(point))<size(data.WorkingFrame.frame_mask_data.BW_Mask_Frame,2) && ceil(Yplus(point))<size(data.WorkingFrame.frame_mask_data.BW_Mask_Frame,1)
                if norm([Xplus(point)-midline_data.midpoints(point,1) Yplus(point)-midline_data.midpoints(point,2)])>norm([X1(point)-...
                    ...
                    && data.WorkingFrame.frame_mask_data.BW_Mask_Frame(floor(Yplus(point)),floor(Xplus(point)))==1 ...
            ...
            if floor(Xminus(point))>1 && floor(Yminus(point))>1 && ceil(Xminus(point))<size(data.WorkingFrame.frame_mask_data.BW_Mask_Frame,2) && ceil(Yminus(point))<size(data.WorkingFrame.frame_mask_data.BW_Mask_Frame,1)       
                if norm([Xminus(point)-midline_data.midpoints(point,1) , Yminus(point)-midline_data.midpoints(point,2)])>norm([X1(point)-...
                    midline_data.midpoints(point,1) Y1(point)-midline_data.midpoints(point,2)]) ...
                    && data.WorkingFrame.frame_mask_data.BW_Mask_Frame(floor(Yminus(point)),floor(Xminus(point)))==1 ...
                ...
                if norm([Xminus(point)-midline_data.midpoints(point,1) , Yminus(point)-midline_data.midpoints(point,2)])>norm([X2(point)-...
                    midline_data.midpoints(point,1) Y2(point)-midline_data.midpoints(point,2)]) ...
                    && data.WorkingFrame.frame_mask_data.BW_Mask_Frame(floor(Yminus(point)),floor(Xminus(point)))==1 ...midlinemidline_data.midpoints(:,2)
