function FrameMaskdata = create_mask(data,S,k)
    if nargin < 3
    ...
    if ~isfield(data,'EnhancedFrame')||isempty(data.EnhancedFrame)
            WorkingFrame=data.SplitChannels.RawGFP{k};
    else
        WorkingFrame = data.EnhancedFrame;
    end
