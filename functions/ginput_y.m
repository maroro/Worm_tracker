...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    while how_many ~= 0
        ...
        if(ptr_fig == fig)
            ...
            axes_handle = gca;            
            if ~(isa(axes_handle,'matlab.graphics.axis.AbstractAxes'))
...
if isWebFigureType(fig, 'UIFigure')
    for k = 1:4
        crossHair(k) = uilabel(fig, 'Visible', 'off', 'BackgroundColor', [0.9290, 0.6940, 0.1250], 'HandleVisibility', 'off'); %#ok<AGROW>
    end
else
    for k = 1:4
        crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels', 'BackgroundColor', [0.9290, 0.6940, 0.1250], 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
    end
end
